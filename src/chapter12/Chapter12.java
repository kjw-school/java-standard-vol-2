...
public class Chapter12 {
	 ...
	 * 타입 안정성을 높인다는 것은 의도하지 않은 타입의 객체가 저장되는 것을 막고, 저장된 객체를 꺼내올 때 원래의 타입과 다른 타입으로 잘못 형변환되어 발생할 수 있는 오류를 줄여준 다는 뜻이다.<br>
	 * 지네릭스의 장점<br>
	 * <pre><code>
	 *		1. 타입 안정성을 제공한다.
	 ...
	 * </code></pre>
	 * <br>
	 * 이 클래스를 지네릭 클래스로 변경하면 다음과 같이 클래스 옆에 '&lt;T&gt;'를 붙이면 된다. 그리고 'Object'를 모두 'T'로 바꾼다.<br>
	 * <pre><code>
	 *     class Box<T> {
	 *         T item;
	 ...
	 * </code></pre>
	 * <br>
	 * Box&lt;T&gt;에서 T를 '타입 변수(type variable)'라고 하며, 'Type'의 첫 글자에서 따온 것이다.<br>
	 * 타입 변수는 T가 아닌 다른 것을 사용해도된다. ArrayList&lt;E&gt;의 경우, 타입 변수 E는 'Element(요소)'의 첫글자를 따서 사용했다.<br>
	 * 타입 변수가 여러 개인 경우에는 Map&lt;K,V&gt;와같이 콤마','를 구분자로 나열하면 된다.<br>
	 * K는 Key(키)를 의미하고, V는 Value(값)을 의미한다. 무조건 'T'를 사용하기보다 가능하면,이처럼 상황에 맞게 의미있는 문자를 선택해서 사용하는 것이 좋다.<br>
	 * 이들은 기호만 다를 뿐 '임이의 참조형 타입'을 의미한다는 것은 모두 같다.<br>
	 * 기존에는 다양한 종류의 타입을 다루는 메서드의 매개변수나 리턴타입으로 Object타입의 참조변수를 많이 사용했고,<br>
	 * 그로 인해 형변환이 불가피했지만, 이젠 Object타입 대신 원하는 타입을 지정하기만 하면 되는 것이다.<br>
	 * 이제 지네릭 클래스가 된 Box클래스의 객체를 생성할 때는 다음과 같이 참조변수와 생성자에 타입 T대신에 사용될 실제 타입을 지정해주어야한다.<br>
	 * <pre><code>
	 ...
	 * </code></pre>
	 * 위의 코드에서 타입 T 대신에 String타입을 지정해줬으므로, 지네릭 클래스 Box&lt;T&gt;는 다음과 같이 정의된 것과 같다.
	 * <pre><code>
	 ...
	 * <pre><code>
	 *     Box<String> b = new box<String>();, String: 대입된 타입(매개변수화된 타입, parameterized type), Box<String>: 지네릭 타입 호출
	 * </code></pre>
	 */
	class Memo03{}
	 ...
	 * 		}
	 * </code></pre>
	 * 지네릭 배열을 생성할 수 없는 것은 new연산자 때문인데, 이 연산자는 컴파일 시점에 타입 T가 뭔지 정확히 알아야 한다.<br>
	 * instanceof연산자도 new연산자와 같은 이유로 T를 피연산자로 사용할 수 없다.<br>
