 ...
 *         </table>
	 ...
	 *     <small>※애너테이션에도 인터페이스처럼 상수를 정의할 수 있지만, 디폴트 메서드는 정의할 수 없다.</small><br>
	 *     애너테이션의 요소는 반환값이 있고 매개변수는 없는 추상 메서드의 형태를 가지며, 상속을 통해 구현하지 않아도 된다.<br>
	 *     다만, 애너테이션을 적용할 때 이 요소들의 값을 빠짐없이 지정해주어야한다. 요소의 이름도 같이 적어주므로 순서는 상관없다.<br>
	 *     애너테이션의 각 요소는 기본값을 가질 수 있으며, 기본값이 있는 요소는 애너테이션을 적용할 때 값을 지정하지 않으면 기본값이 사용된다.<br>
	 *     <small>※기본값으로 null을 제외한 모든 리터럴이 가능하다.</small><br>
	 *     애너테이숀 요소가 하나뿐이고 이름이 value인 경우, 애너테이션을 적용할 때 요소의 이름을 생략하고 값만 적어도 된다.<br>
	 *     요소의 타입이 배열인 경우, 괄호{}를 사용해서 여러 개의 값을 지정할 수 있다.<br>
	 *     기본값을 지정할 때도 마찬가지로 괄호{}를 사용할 수 있다.<br>
	 *     요소의 타입이 배열일 때도 요소의 이름이 value이면, 요소의 이름을 생략할 수 있다. 그래서 애너테이션을 적용할 때 요소의 이름을 생략할 수 있는 것이다.<br>
	 * </p>
	 */
	...
	/**
	 * <p>
	 *     <h5>{@code java.lang.annotation.Annotation}</h5><br>
	 *     모든 애너테이션의 조상은 Annotation이다. 그러나 애너테이션은 상속이 허용되지 않으므로 아래와 같이 명시적으로 Annotation을 조상으로 지정할 수 없다.
	 *     <pre>
	 * 	   &commat;interface TestInfo <b>extends Annotation</b> { // 에러. 허용되지 않는 표현
	 * 			int count();
	 * 			String testedBy();
	 * 		}
	 *     </pre>
	 *     <br>
	 *     모든 애너테이션의 조상인 Annotation인터페이스에 equals(), hashCode(), toString() 과 같은 메서드들이 호출되어 있어<br>
	 *     모든 애너테이션 객체에 대해 해당 메서드들이 호출 가능한 것이다.
	 * </p>
	 */
	private class Memo17{}

	/**
	 * <p>
	 *     <h5>마커 애너테이션 Marker Annotation</h5><br>
	 *     값을 지정할 필요가 없는 경우, 애너테이션의 요소를 하나도 정의하지 않을 수 있다.<br>
	 *     {@code Serializable}이나 {@code Cloneable}인터페이스처럼, 요소가 하나도 정의되지 않은 애너테이션을 마커 애너테이션이라고 한다.
	 * </p>
	 */
	private class Memo18{}

	/**
	 * <p>
	 *     <h5>애너테이션 요소의 규칙</h5><br>
	 *     <b>- 요소의 타입은 기본형, String, enum, 애너테이션, Class만 허용된다.</b><br>
	 *     <b>- ()안에 매개변수를 선언할 수 없다.</b><br>
	 *     <b>- 예외를 선언할 수 없다.</b><br>
	 *     <b>- 요소를 타입 매개변수로 정의할 수 없다.</b><br>
	 *     모든 클래스파일은 클래스로더(Classloader)에 의해 메모리에 올라갈 때, 클래스에 대한 정보가 담긴 객체를 생성하는데 이 객체를<br>
	 *     클래스 객체라고한다. 이 객체를 참조할 때는 '클래스이름.class'의 형식을 사용한다.
	 * </p>
	 */
	private class Memo19{}
}
